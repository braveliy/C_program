/************************************
 *  Fun function：  获取本月月底时间
 *  Pass Parameters：  pin   -- 当前日期 <YYYYMM>
 *  Output Parameters：  pout  -- 当前日期的月底时间 <YYYYMMDD>
 *  return：0   -- success
 *      	-1  -- failure
 ************************************/
...
/****************************************
 *      Fun function:  YYYYMMDD -> YYYY-MM-DD
 *      Pass Parameters  :  len -- pDate所指内存长度
 ***************************************/
int DateTrans(char *pDate, int len)
{

	char buf[14+1];
	//printf("DateTrans() is start");
	...
	//printf("before DateTrans(): pDate is [%s]", pDate);
	...
	strcpy(pDate, buf);
	printf("After DateTrans,pDate[%s]",pDate);
	
	return 0;
}

/****************************************
 *      Fun function:  YYYY-MM-DD -> YYYYMMDD
 *    Pass Parameters  :  len -- pDate所指内存长度
 ***************************************/
int DateTrans_(char *pDate, int len)
{
	...
	memset(buf, 0x00, sizeof(buf));
	//printf("before DateTrans_(): pDate is [%s]", pDate);
	strncpy(buf, pDate, 4);
	...
	//printf("DateTrans_() is end");

	return 0;
}

/****************************************
 *      Fun function:  hhmmss -> hh.mm.ss      
 *      Pass Parameters  :  len -- 长度
 ***************************************/
int TimeTrans(char *pTime, int len)
{
	...
	strcpy(pTime, buf);
	printf("After TimeTrans,pTime[%s]",pTime);

	//printf("TimeTrans() is end");
	return 0;
}
...
int TimeTrans_t(char *pTime, int len)
{
  ...
  memset(buf, 0x00, sizeof(buf));
  //printf("before TimeTrans(): pTime is [%s]", pTime);
  ...
  memset(pTime, 0x00, len);
  strcpy(pTime, buf);
  printf("After TimeTrans_t,pTime[%s]",pTime);
...
/***********************************
 *  Fun function：16进制转为10进制
 *  return：  10进制数
 *  Pass Parameters：    16进制数
 *  Output Parameters：      --
 **********************************/
...
/****************************************************
* Fun function: 计算日期2与日期1之间相差天数
*     注：pDate2为NULL时与系统当前时间比较
*             Pass Parameters必须是mktime()支持的日期
*   pDate1    -- YYYYMMDD
*   pDate2    -- YYYYMMDD
*   day     -- 相差天数
*
...
/****************************************************************
 *  Fun function:  json报文去空格                                     *
 *  Pass Parameters：   sPack -- json报文                                  *
 *                                                              *
 *  return:  int                                                *
 *           0  -- sucess                                         *
 *           !0  -- failure                                       *
